...
function DiceMaker(diceQty) {
    ...
    for (let i = 1; i <= diceQty; i++) {
        ...
        cubeDiv.addEventListener("transitionstart", () => {
            document.dispatchEvent(animStarted);
            console.log("transitionstart for cube_" + i);
        }, true);
        cubeDiv.addEventListener("transitionend", () => {
